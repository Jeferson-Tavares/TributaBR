...
import {
  X, Check, ArrowRight, ChevronDown, AlertTriangle,
  Layers, Shuffle, Globe, ReceiptText, ShieldCheck, Banknote,
} from "lucide-react";

// ─── Tipo de seção ────────────────────────────────────────────────────
type Section = "visao" | "impostos" | "calculoBase" | "competencia" | "cumulatividade" | "transparencia";
...
// ─── Card Imposto Antigo ──────────────────────────────────────────────
function OldTaxCard({ tax, expanded, onClick }: { tax: typeof OLD_TAXES[0]; expanded: boolean; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 p-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 ${c.badge}`}>
          {tax.sigla}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-slate-800">{tax.sigla}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${c.badge}`}>{tax.esfera}</span>
          </div>
      ...
      {expanded && (
        <div className={`px-4 pb-4 border-t ${c.border} animate-fade-in`}>
          <p className="text-sm text-slate-600 mt-3 leading-relaxed">{tax.desc}</p>
  ...
  return (
    <div className={`border-2 rounded-2xl p-5 ${c.border} ${c.bg} card-lift`}>
      <div className="flex items-center gap-3 mb-3">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-base ${c.badge}`}>
          {tax.sigla}
        </div>
        <div>
      ...
      {tax.substitui.length > 0 && (
        <div className="mb-3">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {tax.substitui.map(s => (
              <span key={s} className="text-xs bg-white border border-red-200 text-red-600 line-through px-2 py-0.5 rounded-full font-medium">
                {s}
              </span>
            ))}
          </div>
...
function ConceptComparison({
  icon,
  title,
  oldTitle, oldDesc, oldExample, oldBad,
  newTitle, newDesc, newExample, newGood,
}: {
  icon: React.ReactNode;
  title: string;
  oldTitle: string; oldDesc: string; oldExample: string; oldBad: string;
  newTitle: string; newDesc: string; newExample: string; newGood: string;
}) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
          ...
          <p className="text-sm text-slate-600 leading-relaxed mb-3">{oldDesc}</p>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-bold text-emerald-700">{newTitle}</span>
          </div>
          <p className="text-sm text-slate-600 leading-relaxed mb-3">{newDesc}</p>
...
const INNER_TABS: { id: Section; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "impostos",       label: "Os Impostos",       icon: <ReceiptText className="w-4 h-4" /> },
  { id: "calculoBase",    label: "Como é Calculado",  icon: <Banknote className="w-4 h-4" /> },
  { id: "competencia",    label: "Quem Recebe",       icon: <Globe className="w-4 h-4" /> },
  { id: "cumulatividade", label: "Efeito Cascata",    icon: <Shuffle className="w-4 h-4" /> },
  { id: "transparencia",  label: "Transparência",     icon: <ShieldCheck className="w-4 h-4" /> },
];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-3xl p-6 md:p-8">
        <div className="max-w-3xl">
          <p className="text-xs font-bold text-blue-300 uppercase tracking-widest mb-2">EC 132/2023 — Entenda de vez</p>
          <h2 className="text-2xl md:text-3xl font-black text-white leading-tight mb-3">
            O Sistema Tributário Brasileiro<br />
            <span className="text-blue-300">está mudando — e muito.</span>
          </h2>
          <p className="text-slate-300 text-sm leading-relaxed max-w-xl">
            Por décadas o Brasil teve um dos sistemas tributários mais complexos do mundo. 
            A Reforma Tributária aprovada em 2023 simplifica radicalmente essa estrutura:
            de <strong className="text-white">5 impostos confusos</strong> para <strong className="text-white">2 tributos claros</strong>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
          {[
            ...
            { num: "2026", label: "Início da mudança", color: "text-emerald-300" },
          ].map(s => (
      ...
      {activeSection === "visao" && (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs text-red-400 font-bold uppercase tracking-wide">Antes da Reforma</p>
                  <h3 className="text-base font-black text-red-800">Como Era</h3>
                </div>
              </div>
              <ul className="space-y-2.5">
                {[
                  { icon: "😤", text: "5 impostos diferentes, cada um com suas regras" },
                  { icon: "📚", text: "Centenas de exceções e regimes especiais" },
                  { icon: "⚔️",  text: "27 estados com ICMS diferente — guerra fiscal" },
                  { icon: "🔄", text: "Imposto sobre imposto — efeito cascata" },
                  { icon: "🙈", text: "Alíquota escondida — consumidor não sabia quanto pagava" },
                  { icon: "🏛️", text: "Empresas pagavam para equipes inteiras só de tributos" },
                ].map(item => (
              ...
              <div className="mt-4 p-3 bg-red-100 rounded-xl">
                <p className="text-xs text-red-700 font-semibold">
                  ...
                  empresas gastavam em média <strong>1.500 horas/ano</strong> só para calcular impostos.
                </p>
              </div>
            </div>
            ...
            <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <p className="text-xs text-emerald-500 font-bold uppercase tracking-wide">A partir de 2026</p>
                  <h3 className="text-base font-black text-emerald-800">Como Fica</h3>
                </div>
              </div>
              <ul className="space-y-2.5">
                {[
                  { icon: "✅", text: "2 tributos principais: CBS (federal) + IBS (estado/município)" },
                  { icon: "🇧🇷", text: "Uma regra nacional única para todos os estados" },
                  { icon: "🔗", text: "Crédito automático: empresas não pagam imposto em cascata" },
                  { icon: "📊", text: "Alíquota visível na nota fiscal — total transparência" },
                  { icon: "⚖️",  text: "Fim da guerra fiscal entre estados" },
                  { icon: "🌍", text: "Modelo adotado por mais de 170 países (IVA)" },
                ].map(item => (
                  <li key={item.text} className="flex items-start gap-2.5 text-sm text-slate-600">
                    <span className="text-base flex-shrink-0 mt-0.5">{item.icon}</span>
                    <span>{item.text}</span>
              ...
              <div className="mt-4 p-3 bg-emerald-100 rounded-xl">
                <p className="text-xs text-emerald-700 font-semibold">
                  ✓ Estimativa do governo: economia de até <strong>1,7% do PIB</strong> ao ano com a simplificação.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-700 mb-5 text-center">O que muda na prática</h3>
            <div className="flex flex-col md:flex-row items-center justify-center gap-3">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {OLD_TAXES.map(t => {
                  ...
                  return (
                    <div key={t.sigla} className={`px-3 py-2 rounded-xl text-xs font-bold border ${c.border} ${c.bg} ${c.text}`}>
                      {t.sigla}
                      <span className={`ml-1 text-xs font-normal opacity-70`}>{t.esfera}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs text-slate-400 font-medium">Reforma</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {NEW_TAXES.map(t => {
                  const c = colorMap[t.cor];
                  return (
                    <div key={t.sigla} className={`px-4 py-2.5 rounded-xl text-sm font-black border-2 ${c.border} ${c.bg} ${c.text}`}>
                      {t.sigla}
                      {t.substitui.length > 0 && (
                        <span className="text-xs font-normal ml-1 opacity-60">substitui {t.substitui.join(", ")}</span>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-slate-700">Sistema Antigo — 5 Tributos</h3>
              </div>
              <div className="space-y-2">
                {OLD_TAXES.map(tax => (
                  <OldTaxCard
                    ...
                    onClick={() => setExpandedOld(expandedOld === tax.sigla ? null : tax.sigla)}
                  />
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-slate-700">Sistema Novo — 3 Tributos</h3>
              </div>
              <div className="space-y-3">
                {NEW_TAXES.map(tax => (
      ...
      {activeSection === "cumulatividade" && (
        <div className="space-y-4 animate-fade-in">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-base font-bold text-slate-800 mb-2">O que é o "Efeito Cascata"?</h3>
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              No sistema antigo, o imposto incidia sobre o preço total do produto em <strong>cada etapa da cadeia</strong>, 
              mesmo que o imposto já tivesse sido pago antes. Resultado: o consumidor final pagava imposto sobre imposto sobre imposto.
              No novo sistema, cada empresa desconta o que já pagou — paga apenas sobre o <em>valor que ela agregou</em>.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <X className="w-4 h-4 text-red-500" />
                  <span className="text-sm font-bold text-red-700">Sistema Antigo — Cumulativo</span>
                </div>
                <div className="space-y-2">
                  {[
                    { name: "Extrator de matéria-prima", venda: 100, imposto: 10, acumulado: 10 },
                    { name: "Fábrica", venda: 200, imposto: 20, acumulado: 30, note: "Paga imposto sobre os R$200, mas R$10 já foram pagos antes!" },
                    { name: "Distribuidor", venda: 350, imposto: 35, acumulado: 65, note: "Paga sobre os R$350 — imposto em cascata!" },
                    { name: "Varejista", venda: 500, imposto: 50, acumulado: 115, note: "Consumidor final paga imposto sobre imposto 3x" },
                  ].map((step, i) => (
                    <div key={i} className={`rounded-xl p-3 border ${i < 3 ? "bg-red-50 border-red-100" : "bg-red-100 border-red-200"}`}>
                      <div className="flex justify-between items-start">
                        <span className="text-xs font-semibold text-slate-700">{step.name}</span>
                        <div className="text-right">
                          <span className="text-xs text-slate-400">venda R${step.venda}</span>
                          <span className="text-xs font-bold text-red-600 ml-2">+R${step.imposto} imposto</span>
                        </div>
                      </div>
                      {step.note && <p className="text-xs text-red-500 mt-1">{step.note}</p>}
                    </div>
                  ))}
                  <div className="bg-red-600 text-white rounded-xl p-3 text-center">
                    <p className="text-xs opacity-80">Total de impostos pagos na cadeia</p>
                    <p className="text-lg font-black">R$ 115 = 23% do preço final</p>
                    <p className="text-xs opacity-70">Imposto real muito maior que a alíquota nominal de 10%</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Check className="w-4 h-4 text-emerald-600" />
                  <span className="text-sm font-bold text-emerald-700">Sistema Novo — Não Cumulativo</span>
                </div>
                <div className="space-y-2">
                  {[
                    { name: "Extrator de matéria-prima", venda: 100, debito: 10, credito: 0, recolhe: 10 },
                    { name: "Fábrica", venda: 200, debito: 20, credito: 10, recolhe: 10, note: "Desconta os R$10 pagos na etapa anterior ✓" },
                    { name: "Distribuidor", venda: 350, debito: 35, credito: 20, recolhe: 15, note: "Paga apenas sobre o valor que adicionou ✓" },
                    { name: "Varejista", venda: 500, debito: 50, credito: 35, recolhe: 15, note: "Recolhe só sobre o que ele agregou ✓" },
                  ].map((step, i) => (
                    <div key={i} className={`rounded-xl p-3 border ${i < 3 ? "bg-emerald-50 border-emerald-100" : "bg-emerald-100 border-emerald-200"}`}>
                      <div className="flex justify-between items-start">
                        <span className="text-xs font-semibold text-slate-700">{step.name}</span>
                        <div className="text-right">
                          <span className="text-xs text-blue-500">débito R${step.debito}</span>
                          <span className="text-xs text-slate-400 mx-1">−</span>
                          <span className="text-xs text-purple-500">crédito R${step.credito}</span>
                          <span className="text-xs font-bold text-emerald-600 ml-2">= R${step.recolhe}</span>
                        </div>
                      </div>
                      {step.note && <p className="text-xs text-emerald-600 mt-1">{step.note}</p>}
                    </div>
                  ))}
                  <div className="bg-emerald-600 text-white rounded-xl p-3 text-center">
                    <p className="text-xs opacity-80">Total de impostos pagos na cadeia</p>
                    <p className="text-lg font-black">R$ 50 = 10% do preço final</p>
                    <p className="text-xs opacity-70">Exatamente a alíquota nominal. Sem cascata!</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4 animate-fade-in">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                <X className="w-4 h-4 text-red-500" />
                <h3 className="text-sm font-bold text-red-700">Nota Fiscal Hoje (opaca)</h3>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 font-mono text-xs space-y-1.5">
                <div className="text-slate-400 text-center border-b border-slate-200 pb-2 mb-2">DANFE — Documento Auxiliar</div>
                <div className="flex justify-between"><span>Produto</span><span>R$ 1.000,00</span></div>
                <div className="flex justify-between text-slate-400"><span>Base ICMS..........</span><span>R$ 1.000,00</span></div>
                <div className="flex justify-between text-slate-400"><span>Valor ICMS.........</span><span>R$ 170,00</span></div>
                <div className="flex justify-between text-slate-400"><span>Base PIS/Cofins...</span><span>R$ 1.000,00</span></div>
                <div className="flex justify-between text-slate-400"><span>PIS................</span><span>R$ 16,50</span></div>
                <div className="flex justify-between text-slate-400"><span>Cofins.............</span><span>R$ 76,00</span></div>
                <div className="flex justify-between font-bold border-t border-slate-200 pt-2 mt-2"><span>TOTAL</span><span>R$ 1.000,00</span></div>
                <p className="text-red-400 text-center mt-2">⚠ Os impostos ficam "dentro" do preço. O total parece R$ 1.000,00 mas o produto custaria R$ 737,50 sem impostos!</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-emerald-200 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                <Check className="w-4 h-4 text-emerald-600" />
                <h3 className="text-sm font-bold text-emerald-700">Nota Fiscal Nova (transparente)</h3>
              </div>
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 font-mono text-xs space-y-1.5">
                <div className="text-emerald-600 text-center border-b border-emerald-200 pb-2 mb-2">DANFE — IVA Dual</div>
                <div className="flex justify-between"><span>Produto (preço base)</span><span>R$ 1.000,00</span></div>
                ...
                <p className="text-emerald-600 text-center mt-2">✓ Qualquer pessoa pode verificar: produto custa R$ 1.000 + R$ 265 de impostos = R$ 1.265 total.</p>
              </div>
            </div>
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-blue-800 mb-2">Por que a transparência importa?</h3>
            <div className="grid md:grid-cols-3 gap-3">
              {[
                { icon: "👁️", title: "Cidadão informado", desc: "Você saberá exatamente quanto paga de imposto em cada compra — como acontece na Europa." },
                { icon: "⚖️",  title: "Democracia fiscal", desc: "Com os impostos visíveis, o debate sobre alíquotas se torna mais justo e transparente." },
                { icon: "🏪", title: "Concorrência justa", desc: "Empresas não conseguem esconder benefícios tributários — campo de jogo nivelado." },
              ].map(item => (
                <div key={item.title} className="bg-white rounded-xl p-4 border border-blue-100">
                  <span className="text-2xl">{item.icon}</span>
                  <h4 className="text-sm font-bold text-slate-700 mt-2 mb-1">{item.title}</h4>
